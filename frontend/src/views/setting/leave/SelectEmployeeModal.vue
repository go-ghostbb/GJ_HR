...
<script lang="ts" setup>
  ...
  import { ResetAvailableParams } from '@/api/setting/model/leaveModel';
  import dayjs from 'dayjs';
  import { resetAvailable } from '@/api/setting/leave';
  ...
  //-準備設定的資料
  const formData = ref<ResetAvailableParams>({
    leaveId: [],
    employeeId: [],
    year: dayjs().year(),
  });
  ...
  const handleOk = async () => {
    try {
      ...
      formData.value.employeeId = filterID(checkedKeys.value);
      await resetAvailable(formData.value);
