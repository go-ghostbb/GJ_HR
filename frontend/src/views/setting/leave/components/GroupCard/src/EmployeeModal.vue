...
<script lang="ts" setup>
  ...
  import { setLeaveGroupEmployee } from '@/api/setting/leave';
  ...
  const [registerModal, { closeModal, changeLoading, setModalProps }] = useModalInner(
    async (data) => {
      ...
      const checkedInLeave: number[] = data.checkedInLeave.filter((n: number) => {
      ...
      employees.forEach((employee) => {
        //-判斷此員工是否可呈現在畫面
        if (checkedInLeave.indexOf(employee.ID) > -1) {
          return;
        }
  ...
  const handleOk = () => {
    ...
    setLeaveGroupEmployee(leaveGroupId.value, filterID(checkedKeys.value))
      .then(() => {
