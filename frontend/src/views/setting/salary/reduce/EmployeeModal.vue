...
<script lang="ts" setup>
  ...
  import { setSalaryReduceItemEmployee } from '@/api/setting/salaryItem';
  ...
  const reduceItemId = ref<number>(0);
  ...
  const [registerModal, { closeModal, changeLoading, setModalProps }] = useModalInner(
    async (data) => {
      ...
      reduceItemId.value = data.id;

      checkedKeys.value = data.employeeIds;
  ...
  const handleOk = async () => {
    try {
      ...
      await setSalaryReduceItemEmployee(reduceItemId.value, empIDs);
      emit('success');
      closeModal();
    } finally {
