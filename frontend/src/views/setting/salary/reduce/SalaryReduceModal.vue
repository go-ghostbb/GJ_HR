...
<script lang="ts" setup>
  ...
  import {
    ...
    SalaryCalcUnit,
    Operator,
  } from '@/api/setting/model/salaryReduceItemModel';
  import { createSalaryReduceItem, updateSalaryReduceItem } from '@/api/setting/salaryItem';
  ...
  //-form data
  const formData = reactive<SalaryReduceItemModel>({ ID: 0 });

  //-modal標題
  const getTitle = computed(() => (!unref(isUpdate) ? '新增減項' : '編輯減項'));
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (unref(isUpdate)) {
        //-編輯
        await updateSalaryReduceItem(salaryItemID.value, formData);
      } else {
        //-新增
        await createSalaryReduceItem(formData);
      }
      emit('success');
      closeModal();
    } finally {
   ...
   * @param value
   */
  const setValue = (value: SalaryReduceItemModel) => {
