    ...
    gteDay: [
      {
        validator: (rule: Rule) => {
          ...
          const value = formState.setting[Number(sp[1])]['gteDay'];
          ...
          if (!value || value.toString().replace(/[^\d]/g, '') === '') {
            formState.setting[Number(sp[1])][sp[0]] = null;
            return Promise.reject('請輸入觸發天數');
          } else {
            formState.setting[Number(sp[1])][sp[0]] = Number(
